...
export class DataSource {
    ...
    async getBalance(address) {
        let balanceResponse;
        try {
            balanceResponse = await this.gwClient.makeQuery(
                "getBalance",
                address,
                0
            );
    ...
    async getTx(txHash) {
        let txResponse;
        try {
            txResponse = await this.gwClient.makeQuery(
                "getTransaction",
                txHash
            );
        } catch (error) {
    ...
    async sendTransaction(ppk, passphrase, toAddress, amount) {
        ...
        const accountOrUndefined =
            await this.__pocket.keybase.importPPKFromJSON(
                passphrase,
                ppk,
                passphrase
            );
        ...
        const transactionSenderOrError =
            await this.__pocket.withImportedAccount(
        ...
        try {
            ...
            console.log(
                `Failed to send transaction with error: ${error.raw_log}`
            );
    ...
    async getAllTransactions(address) {
        ...
        if (receivedTxs === undefined && sentTxs === undefined)
            return undefined;
    ...
    async getTxs(address, received) {
        ...
        try {
            txs = await this.gwClient.makeQuery(
                "getAccountTxs",
                address,
                received,
                false,
