...
describe("DataSource", () => {
    ...
    describe("DataSource#getBalance", () => {
        test("responds with a number as balance", async () => {
            const balance = await context.datasource.getBalance(
                ...context.args.getBalance
            );
    ...
    describe("DataSource#getNode", () => {
        test("responds with a node as an object", async () => {
            const node = await context.datasource.getNode(
                ...context.args.getNode
            );
    ...
    describe("DataSource#getTxs", () => {
        test("responds with a txs as an array", async () => {
            ...
            expect(Array.isArray(txs.txs)).toBe(true);
        });
    ...
    describe("DataSource#getAllTransactions", () => {
        test("responds with a txs as an array", async () => {
            const txs = await context.datasource.getAllTransactions(
                ...context.args.getAllTransactions
            );
            ...
            expect(types.some((t) => ["Received", "Sent"].includes(t))).toBe(
                true
            );
        });
    ...
    describe.only("DataSource#sendTransaction", () => {
        test("responds with a txhash and a log resuming the transaction", async () => {
            const response = await context.datasource.sendTransaction(
                ...context.args.sendTransaction()
            );
