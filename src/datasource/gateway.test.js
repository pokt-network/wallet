...
import { processTx } from "./gateway-test-utils";
...
describe("GatewayClient", () => {
    ...
    test("instantiates properly", () => {
        ...
        expect(context.client.config.baseUrl).toEqual(testConfig.gatewayUrl);

        expect(context.client.config.headers).toEqual(testConfig.http.headers);
    });
    ...
    const runOnlyWhitelistedTestCases = (queryAnnotation) =>
        whitelistedTestCases.includes(queryAnnotation.queryName);
    ...
    const testQuery = ({ queryName, responseProperties, args, getArgs }) => {
        describe(`GatewayClient.${queryName}`, () => {
            test("is functional", async () => {
                ...
                await expect(
                    context.client.makeQuery(queryName, ..._args)
                ).resolves.not.toThrow();
            });

            test("returns proper response format on success", async () => {
                const _args = args !== null ? args : getArgs();
                const response = await context.client.makeQuery(
                    queryName,
                    ..._args
                );

                responseProperties.onSuccess.forEach((prop) => {
                    if (typeof prop === "string") {
                        ...
                        expect(response).toHaveProperty(parentProp);

                        nestedProps.forEach((nestedProp) => {
                            expect(response[parentProp][0]).toHaveProperty(
                                nestedProp
                            );
                        });
                    }
            ...
            test.skip("returns proper response format on error", async () => {
                ...
                responseProperties.onFailure.forEach((prop) => {
                    expect(response).toHaveProperty(prop);
                });
            });
        });
    ...
    testCases.filter(runOnlyWhitelistedTestCases).forEach(testQuery);
    ...
    describe("GatewayClient.sendRawTx", () => {
        test("is functional and returns proper response format on success", async () => {
            const { queryName, args, getArgs, responseProperties } =
                testCases.filter(
                    (queryAnnotation) =>
                        queryAnnotation.queryName === "sendRawTx"
                )[0];
